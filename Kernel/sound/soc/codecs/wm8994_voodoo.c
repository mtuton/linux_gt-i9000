...
bool fm_radio_headset_normalize_gain = false;
#endif
...
void update_fm_radio_headset_restore_freqs(bool with_mute)
{
	...
	if (fm_radio_headset_restore_bass) {
		...
		/*wm8994_write(codec, WM8994_AIF2_ADC_FILTERS,
			     WM8994_AIF2ADCL_HPF |
			     WM8994_AIF2ADCR_HPF);*/
		wm8994_write(codec, WM8994_AIF2_ADC_FILTERS, 0x8000); //disable HPF but enable 4FS
		wm8994_write(codec, WM8994_AIF2_DRC_1, 0x0000); //disable DRC and NG
	} else {
...
void update_fm_radio_headset_normalize_gain(bool with_mute)
{
	...
	if (fm_radio_headset_normalize_gain) {
		...
		wm8994_write(codec, WM8994_LEFT_LINE_INPUT_3_4_VOLUME, 0x4F);
		wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_3_4_VOLUME, 0x14F);
		/*wm8994_write(codec, WM8994_AIF2_DRC_2, 0x0840);
		...
		wm8994_write(codec, WM8994_AIF2_DRC_1, 0x019C);*/
		wm8994_write(codec, WM8994_AIF2_DRC_1, 0x0000); //SpeedMod: Disable DRC
	}
	bypass_write_hook = false;
}
#endif
				      ...
				      unsigned int reg, unsigned int value)
{
	...
	//SpeedMod: Extra settings for audio playback
	if (is_path(HEADPHONES)
		&& !(wm8994->codec_state & CALL_ACTIVE)) {
		
		if (reg == WM8994_AIF1_DRC1_1)
			value = 0x0000; //Disable DRC
		
		if (reg == WM8994_AIF1_DAC1_FILTERS_1)
			value &= ~(WM8994_AIF1DAC1_DEEMP_MASK); //Set masked bits to 0 to disable de-emphasis
